...
const GITHUB_CONFIG = {
    ...
    // Pre-built token index (generated by assets repo update-index workflow)
    indexUrl: "https://github.saturndex.org/index.json",
};
...
// ---------- Cache ----------
const CACHE_KEY = "saturn_tokens_cache";
...
function saveTokensToCache(tokens) {
    try {
...
async function loadTokensFromGitHub() {
    ...
    // 1. Try the pre-built index first: 1 request instead of ~900.
    try {
        spinner.classList.remove("hidden");
        grid.innerHTML = "";

        const response = await fetch(GITHUB_CONFIG.indexUrl);
        if (response.ok) {
            const index = await response.json();
            if (index && Array.isArray(index.tokens)) {
                allTokens = index.tokens;
                saveTokensToCache(allTokens);
        ...
        console.warn("Index fetch failed, falling back to per-token loading");
    } catch (error) {
        console.warn(
            "Index fetch failed, falling back to per-token loading:",
            error,
        );
    }

    // 2. Fallback: progressive per-token loading (optimized concurrency).
    await loadTokensProgressive();
}

async function loadTokensProgressive() {
    const spinner = document.getElementById("loadingSpinner");
    const grid = document.getElementById("tokenGrid");
    const countEl = document.getElementById("tokenCount");
    const refreshBtn = document.getElementById("refreshTokens");
    ...
    try {
        ...
        // Chain dir listings load in parallel (2 contents-API calls).
        const [ethTokens, etcTokens] = await Promise.all([
            fetchChainTokens("ethereum"),
            fetchChainTokens("classic"),
        ]);
        allTokens = ethTokens.concat(etcTokens);
...
async function fetchChainTokens(chain) {
    ...
    try {
        ...
        // Fetch info.json for each token via the Pages CDN.
        // Higher concurrency (25 vs 10) cuts the number of sequential
        // batch rounds: ~36 instead of ~90 for 900 tokens.
        const CONCURRENCY = 25;
        for (let i = 0; i < tokenDirs.length; i += CONCURRENCY) {
            const batch = tokenDirs.slice(i, i + CONCURRENCY);
...
async function openTokenModal(token) {
    // Index entries carry only browse-list fields (name/symbol/address/chain/logo).
    // Fetch the full info.json on demand so the modal shows decimals + socials.
    let fullToken = token;
    if (token && token.address && token.chain && token.decimals === undefined) {
        try {
            const fullInfo = await fetchTokenInfo(token.chain, token.address);
            if (fullInfo) {
                fullToken = Object.assign({}, token, fullInfo, {
                    chain: token.chain,
                });
            }
        } catch {
            // fall through with index-level data
        }
    }
    currentToken = fullToken;

    document.getElementById("modalTitle").textContent =
        fullToken.name || "Unknown Token";
    document.getElementById("modalSymbol").textContent =
        fullToken.symbol || "???";
    document.getElementById("modalAddress").textContent =
        fullToken.address || "";
    document.getElementById("modalDecimals").textContent =
        `Decimals: ${fullToken.decimals || 0}`;

    const logoUrl =
        fullToken.logo ||
        `https://ui-avatars.com/api/?name=${encodeURIComponent(fullToken.symbol || "T")}&background=3b82f6&color=fff&size=64`;
    ...
    for (const social of socialLinks) {
        if (fullToken[social.key]) {
            const link = document.createElement("a");
            link.href = fullToken[social.key];
            link.target = "_blank";
...
// Toggle the submit button between idle and in-flight "processing" state.
// The label and spinner are static button children; we only toggle visibility
// and the disabled guard, so the button keeps its shape and brand color.
function setSubmitLoading(loading) {
    const submitBtn = document.getElementById("submitBtn");
    const submitSpinner = document.getElementById("submitSpinner");
    submitBtn.classList.toggle("submit-loading", loading);
